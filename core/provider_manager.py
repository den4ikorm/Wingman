...
ProviderManager v3 — Groq основной, Gemini резервный

Порядок:
  1. Groq       — ротация GROQ_API_KEY_1..5  (основной, быстрый)
  2. Gemini     — ротация GEMINI_KEY_1..4    (резервный)
  3. OpenRouter — OPENROUTER_API_KEY          (последний резерв)
"""
    ...
    async def _try_groq(self, system: str, text: str, max_tokens: int) -> str | None:
        ...
        for attempt in range(attempts):
            ...
            try:
                ...
                logger.error(f"Groq error (ключ {attempt+1}): {e}")
                gkm.rotate()
